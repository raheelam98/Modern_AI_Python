...
A) == checks value equality, is checks reference- B) is checks value equality, == checks reference- C) They are the same- D) is is used only for numbers
